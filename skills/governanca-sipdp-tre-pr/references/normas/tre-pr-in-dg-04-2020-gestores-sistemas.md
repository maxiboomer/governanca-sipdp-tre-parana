---
title: "Tre Pr In Dg 04 2020 Gestores Sistemas"
created: 2026-08-26
updated: 2026-08-26
type: fonte-normativa
status: vigente
curadoria: completa
escopo: contextual
status_verificacao: "Compilada no portal oficial; vigência confirmada. Síntese gerada a partir do texto integral em raw/."
sources: [raw/tre-pr-in-dg-04-2020-gestores-sistemas.md]
tags: [norma, tre-pr]
---

# Tre Pr In Dg 04 2020 Gestores Sistemas

## Síntese executiva

Norma vigente do acervo, relevante para segurança da informação, proteção de dados ou governança de TI no TRE-PR/Justiça Eleitoral. **Situação registrada:** `vigente`. Síntese extraída do texto integral em `raw/tre-pr-in-dg-04-2020-gestores-sistemas.md`.

## Objeto / Ementa

Institui e regulamenta as atribuições dos Gestores de Sistemas Informatizados no âmbito daJustiça Eleitoral do Paraná.

## Obrigações e disposições principais

- Art. 1º A presente norma institui e regulamenta as atribuições dos Gestores de Sistemas Informatizados, no âmbito da Justiça Eleitoral do Paraná.
- Art. 2º Para efeito desta norma, considera-se:
- Art. 3º Para cada sistema informatizado será designado um Gestor.
- Art. 5º Compete ao Gestor ou Comissão Gestora do Sistema Informatizado:
- Art. 6º Para os sistemas já existentes que ainda não possuem gestor de sistema indicado, será realizada indicação conforme art. 3º § 4º, no prazo de 180 (cento e oitenta) dias, contados a partir da publicação desta norma.
- Art. 7º A Secretaria de Tecnologia da Informação manterá atualizada na intranet a relação dos gestores de sistema e de seus suplentes.
- Art. 8º Os casos omissos, subsidiados com as informações necessárias encaminhadas pela Secretaria de Tecnologia da Informação, serão submetidos ao Diretor-Geral.
- Art. 9º Esta instrução normativa entra em vigor na data de sua publicação.

## Interpretação operacional

- Esta é uma página de índice analítico; a fonte de verdade é o texto integral em `raw/tre-pr-in-dg-04-2020-gestores-sistemas.md`.
- Separar obrigações, papéis e responsabilidades conforme a norma e suas normas complementares.
- Para citação formal, subir a referência ao DJE/DOU (ver ADR 0002).

## Relações

- [[entities/agm]]

## Fonte e vigência

- Fonte integral: `raw/tre-pr-in-dg-04-2020-gestores-sistemas.md`
- Verificação: 2026-08-26

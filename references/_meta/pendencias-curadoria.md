---
title: "Pendências de curadoria e verificação normativa"
created: 2026-08-26
updated: 2026-08-26
type: metadata
status: não-aplicável
curadoria: completa
escopo: contextual
tags: [metadados, curadoria, vigencia]
---

# Pendências de curadoria e verificação normativa

## Prioridade 1 — SI/PDP e governança diretamente operacional

- Classificação inicial aplicada aos stubs; consultar `wiki/_meta/classificacao-normas.md`.
- Curar prioritariamente as páginas com `escopo: central-si-pdp`.
- Confirmar no DJE/DOU a vigência das normas marcadas como `não-confirmada`.
- Relacionar sucessoras, alterações e revogações com links entre as páginas.

## Prioridade 2 — processos e governança de TI

- Curar somente as normas SECTI e de governança que tenham obrigações, controles, papéis ou prazos atuais.
- Consolidar processos repetidos em conceitos operacionais sem substituir a página da norma.

## Prioridade 3 — arquivo histórico e fora de escopo

- Identificar normas revogadas sem efeito residual e arquivá-las em `_archive/normas/`.
- Manter o texto integral correspondente em `raw/`.
- Não investir em portarias casuísticas, calendários, designações e matérias sem relação com SI/PDP.

## Regra de segurança

Nenhuma ausência de status ou classificação de escopo equivale a uma conclusão jurídica. A confirmação oficial deve preceder qualquer afirmação de vigência.

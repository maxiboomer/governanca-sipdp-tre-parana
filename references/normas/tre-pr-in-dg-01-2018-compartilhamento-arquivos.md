---
title: "INSTRUÇÃO NORMATIVA - DG Nº 01/2018"
created: 2026-08-26
updated: 2026-08-26
type: fonte-normativa
status: revogada
status_verificacao: "A própria fonte declara revogação pela IN 004/2022, art. 28; verificado em 2026-08-26."
fonte_publicacao: "https://apps.tre-pr.jus.br/files/legislacaocompilada/in/2018/in012018dg.html"
data_publicacao: ""
tags: [norma, curadoria, si-pdp]
sources: [raw/tre-pr-in-dg-01-2018-compartilhamento-arquivos.md]
curadoria: completa
escopo: contextual
---

# INSTRUÇÃO NORMATIVA - DG Nº 01/2018

## Finalidade e escopo

Esta norma integra o corpus de **Governança/serviços de TI**. A página foi curada a partir do texto integral preservado em `raw/tre-pr-in-dg-01-2018-compartilhamento-arquivos.md`; a fonte bruta permanece imutável.

A finalidade específica deve ser interpretada diretamente com o texto integral e as normas supervenientes.

## Status normativo

**Revogada — art. 28 da IN TRE/PR/DG nº 004/2022**

- Verificação: **não substituir esta indicação pela aparência do inventário**; consultar a publicação oficial antes de emitir parecer ou orientar procedimento.
- Norma sucessora/efeito: Revogada — art. 28 da IN TRE/PR/DG nº 004/2022.

## Pontos operacionais extraídos

- Art. 1º Ficam estabelecidos os critérios para
- Art. 2º A Secretaria de Tecnologia da Informação
- Art. 3º As permissões de acesso aos serviços deverão
- Art. 4º Serão estabelecidas quotas de armazenamento
- Art. 5º Serão efetuadas cópias de segurança dos
- Art. 6º Os recursos computacionais disponibilizados
- Art. 7º É vedado o armazenamento de arquivo
- Art. 8º Os arquivos armazenados poderão ser


## Relações

[[wiki/concepts/seguranca-informacao-justica-eleitoral]]
- [[wiki/entities/secti]]

## Fonte integral

- `raw/tre-pr-in-dg-01-2018-compartilhamento-arquivos.md`
- Publicação/URL registrada: https://apps.tre-pr.jus.br/files/legislacaocompilada/in/2018/in012018dg.html

## Nota de qualidade

Página anteriormente gerada como stub. Esta primeira curadoria não constitui certificação de vigência: o campo `status_verificacao` explicita a pendência de confirmação oficial.
